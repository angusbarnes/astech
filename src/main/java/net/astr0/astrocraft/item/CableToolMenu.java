...
public class CableToolMenu extends AbstractContainerMenu {
    ...
    public CableToolMenu(int id, Inventory playerInv, FriendlyByteBuf buf) {
        this(id, playerInv, buf.readItem());
    }

    public CableToolMenu(int id, Inventory playerInv, ItemStack stack) {
        super(ModMenuTypes.CABLE_TOOL_MENU.get(), id);

        // Retrieve the inventory handler from the capability
        LazyOptional<IItemHandler> lazy = CableLayingToolItem.getHandler(stack);
        this.handler = (ItemStackHandler) lazy.orElseThrow(() -> new IllegalStateException("No item handler present"));
        ...
        for (int i = 0; i < 2; ++i) {
            for (int l = 0; l < 9; ++l) {
                this.addSlot(new SlotItemHandler(handler, l + i * 9, 8 + l * 18, 16 + i * 18));
            }
        }
    ...
    private static final int TE_INVENTORY_INPUT_COUNT = 18;
    @Override
